...
test.describe('crew hatches', () => {
  ...
  test('a lid opens, travels, and settles', async ({ page }) => {
    const before = await page.evaluate(() => {
      const a = window.__TIGER__!.internals.articulations[0]!;
      return { open: a.isOpen, fraction: a.openFraction };
    });
    expect(before.open).toBe(false);
    expect(before.fraction).toBe(0);

    await page.evaluate(() => window.__TIGER__!.internals.articulations[0]!.toggle());
  ...
  test('the lid actually moves in the world when it opens', async ({ page }) => {
    const closed = await page.evaluate(() => {
      const m = window.__TIGER__!.internals.articulations[0]!.mesh;
      m.updateMatrixWorld(true);
      return m.matrixWorld.elements.slice(12, 15);
    });

    await page.evaluate(() => {
      window.__TIGER__!.internals.articulations[0]!.setOpen(true);
      window.__TIGER__!.internals.articulations[0]!.settle();
    });
  ...
  test('the player is offered the hatch when they stand on the roof', async ({ page }) => {
    const prompt = await page.evaluate(() => {
      const t = window.__TIGER__!.internals;
      // On the roof, beside the driver's hatch, looking down at it.
      t.player.teleport(new t.THREE.Vector3(0.56, 2.1, 1.2), 0);
      t.player.state.pitch = -0.7;
      window.__TIGER__!.step(2);
      return window.__TIGER__!.interactionPrompt();
    });
